...
updateMap(input.value);parseJSON("./results/results.json")}function updateMap(a){""!=a&&(new google.maps.Geocoder).geocode({address:a},function(a,c){c==google.maps.GeocoderStatus.OK&&map.setCenter(a[0].geometry.location)})}function updateLocation(a){(new google.maps.Geocoder).geocode({latLng:map.getCenter()},function(b,c){if(c==google.maps.GeocoderStatus.OK)for(i=0;i<b.length;i++)for(j=0;j<b[i].types.length;j++)"locality"==b[i].types[j]&&(a.value=b[i].formatted_address)})}
